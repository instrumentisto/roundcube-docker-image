...
@test "PHP ext 'fileinfo' is installed" {
  run docker run --rm --pull never --entrypoint sh $IMAGE -c \
    'php -m | grep -Fx fileinfo'
  [ "$status" -eq 0 ]
}

@test "PHP ext 'filter' is installed" {
  run docker run --rm --pull never --entrypoint sh $IMAGE -c \
    'php -m | grep -Fx filter'
  [ "$status" -eq 0 ]
}
...
@test "SHARE_APP=0 makes /var/www link to /app/ dir" {
  run docker run --rm --pull never \
                 -e SHARE_APP=0 -e PHP_OPCACHE_JIT_BUFFER_SIZE=0 \
                 --entrypoint /docker-entrypoint.sh $IMAGE sh -c \
...
@test "SHARE_APP=1 makes /var/www link to /shared/ dir" {
  run docker run --rm --pull never \
                 -e SHARE_APP=1 -e PHP_OPCACHE_JIT_BUFFER_SIZE=0 \
                 --entrypoint /docker-entrypoint.sh $IMAGE sh -c \
    'test -L /var/www && readlink -f /var/www | tr -d "\n"'
  [ "$status" -eq 0 ]
  [ "$output" == "/shared" ]
